    ...
    try {
      if (mode === "login") {
        ...
        if (password !== confirmPassword) {
          ...
          await onRegister(identifier, email, password);
        }
      } else if (mode === "forgot") {
      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <Card className="bg-ps5-card/80 backdrop-blur-sm border-ps5-secondary/30 p-8 shadow-2xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Identifier input (username or email for login / forgot, username for register) */}
            ...
            <Button
              ...
              className="w-full bg-ps5-accent hover:bg-ps5-accent/90 text-white font-medium py-3 text-base shadow-lg transition-all duration-200"
            >
              {isLoading
                ? (mode === "login"
                    ? "Signing In..."
            ...
            {/* Forgot Password link visible on login mode */}
            {mode === "login" && (
